...
Common causes are:
 ...
 - The hit result contains top-level fields that are {logstash-ref}/processing.html#reserved-fields[reserved in Logstash], but do not have the expected shape. Use the <<plugins-{type}s-{plugin}-target>> directive to avoid conflicts with the top-level namespace.
 - The <<plugins-{type}s-{plugin}-docinfo>> option is enabled, and the docinfo fields cannot be merged into the hit result. Combine <<plugins-{type}s-{plugin}-target>> and <<plugins-{type}s-{plugin}-docinfo_target>> to avoid conflict.

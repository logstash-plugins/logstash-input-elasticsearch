...
{ls} 7.9 and later supports defining an environment variable `hosts =>
'${ES_HOSTS}' with multiple whitespace delimited URIs. 
This plugin automatically expands `hosts` strings that contain multiple
white-space delimited URs into separate entries. 
